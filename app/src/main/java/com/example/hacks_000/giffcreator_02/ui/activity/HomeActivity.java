...
public class HomeActivity extends AppCompatActivity implements MyOnClickListener, FacebookConstant {
                ...
                public void run() {
                    if (mFab != null) {
                        SimpleHintContentHolder blockInfo = new SimpleHintContentHolder.Builder(
                                mFab.getContext()).setContentText(R.string.text_hint_add_create_gif)
                                ...
                                .setMarginByResourcesId(R.dimen.activity_vertical_margin,
                                        ...
                                        R.dimen.activity_horizontal_margin)
                                .build();
                        new HintCase(mFab.getRootView()).setTarget(mFab, new CircularShape())
        ...
        LoginManager.getInstance()
                .registerCallback(mCallbackManager, new FacebookCallback<LoginResult>() {
                    ...
                    public void onSuccess(LoginResult loginResult) {
                            ...
                            public void onCompleted(GraphResponse graphResponse) {
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < lengthAlbum; i++) {
                                        getAllImageLink(
                                                array.getJSONObject(i).getString(FACEBOOK_ID),
                                                lengthAlbum);
                                    }
                        ...
                        GraphRequest req = GraphRequest.newGraphPathRequest(mToken, FACEBOOK_ALBUM,
                                getAlbumIdCallback);
    ...
    private void startFacebookImageActivity() {
        FacebookImageActivity_.intent(getApplicationContext()).flags(Intent.FLAG_ACTIVITY_NEW_TASK).extra(Constant.INTENT_DATA, (ArrayList) mImageLink).start();
    }
    ...
    @AfterViews
    protected void checkPermissionAndPickImage() {
    ...
    private void checkPermissionAndTakePhoto() {
        ...
        if (ContextCompat.checkSelfPermission(HomeActivity.this, Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(HomeActivity.this,
                    new String[] { Manifest.permission.CAMERA }, MY_PERMISSIONS_USE_CAMERA);
        }
    }
    ...
    public void onItemClick(View view, int position) {
        ...
        for (int i = 0; i < length; i++) {
            ListGifAdapter.GifViewHolder viewHolder =
                    (ListGifAdapter.GifViewHolder) mRecyclerView.findViewHolderForAdapterPosition(
                            i);
            if (viewHolder != null) {
                if (i == position) {
